...
export default function SoalTab({ sessionId, onCountChange }: { sessionId: string; onCountChange?: () => void }) {
      ...
      complete: (result) => {
        const rows: CsvRow[] = (result.data as Record<string, string>[]).map((row) => {
          ...
          return {
            ...
            option_e: row.option_e || "", correct_option: row.correct_option || "",
            _valid: errors.length === 0, _error: errors.join(", "),
          } as CsvRow;
        });
        setCsvRows(rows);
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-gray-500">{questions.length} soal</p>
                ...
                {csvRows.map((row, i) => (
                  <div key={i} className={`p-3 rounded-xl border text-sm ${row._valid ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}`}>
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${row._valid ? "bg-green-200 text-green-700" : "bg-red-200 text-red-700"}`}>
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm p-6">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => handleDelete(deleteId)}
                className="flex-1 bg-red-600 text-white py-3 rounded-xl font-semibold hover:bg-red-700">Hapus</button>
            </div>
          </div>
        </div>
      )}
